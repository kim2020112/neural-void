...
土星环、双螺旋和引力奇点拥有专用渲染器。其余场景继续共享通用 `ParticleUniverse` 和形态注册表。
...
场景切换会重新挂载不同的专用渲染器，不尝试在通用粒子和专用几何之间进行 morph。三个专用渲染器使用动态导入；场景库在悬停、聚焦或按下时预取目标模块，模块就绪后才提交场景切换。

## Rendering Boundaries

### ParticleUniverse

通用场景共享 10,000 粒子、通用 shader 和形态缓动。旧的土星、双螺旋和奇点形态生成器仍保留在注册表中，用于兼容形态数据，但运行时由专用渲染器接管。
...
| 输入 | 通用语义 | 土星 | 双螺旋 | 引力奇点 |
|---|---|---|---|---|
| 握拳 | 吸引/压缩 | 环带收缩、涡旋加速 | 半径收紧、螺距压缩 | 吸积盘收缩、螺旋吸入 |
| 张掌 | 排斥/释放 | 环带扩张、冲击 | 双链解旋、碱基键减弱 | 盘面扩张、径向冲击 |
| 指向 | 局部牵引 | 指尖光球和牵引弧 | 锁定序列并发出双向扫描脉冲 | 局部引力透镜和牵引弧 |
| 双拳 | 聚能 | 中心核心与环带尺度 | 双手距离控制链体长度与扭转 | 中心奇点与吸积盘尺度 |
| 稳定后张掌 | 爆发 | 中心轨道爆发 | 复制解旋波由激活位点向两端推进 | 光子环、冲击波和喷流爆发 |
